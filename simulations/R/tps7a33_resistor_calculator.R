calcV = function(R1, R2)
{
    VrefTyp = -1.175
    VrefMax = -1.157
    IfbTyp = 14e-9         # nA, into the device
...
test = expand.grid(
    R1s1 = jlc_resistors, 
    R1s2p1 = jlc_resistors,
    R1s2p2 = c(jlc_resistors, Inf), 
    R2s1 = jlc_resistors, 
    R2s2p1 = jlc_resistors,
...
test$ndevices = 4 + is.finite(test$R1s2p2) + is.finite(test$R2s2p2)
temp = calcV(test$R1, test$R2)
test$Vout = temp$Vout
test$Imin = temp$Imin
test = test[test$Imin >= 5.5e-6 & test$Imin <= 20e-6 & test$Vout >= target_voltage-target_voltage_tolerance & test$Vout <= target_voltage+target_voltage_tolerance & test$ndevices < 5,]
